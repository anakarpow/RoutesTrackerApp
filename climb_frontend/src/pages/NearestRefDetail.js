...
import { fetchNearestRef, fetchRoutes } from "../api";

export default function NearestRefDetail() {
  const { id } = useParams();
  const [nearestRef, setNearestRef] = useState(null);
    ...
    Promise.all([
      ...
      fetchRoutes().then(allRoutes => 
        allRoutes.filter(route => 
          ...
          route.nearest_reference.id === parseInt(id)
        )
      )
    ])
    .then(([refData, refRoutes]) => {
      setNearestRef(refData);
      setRoutes(refRoutes.map(r => ({
        id: r.id,
        name: r.name,
        type: r.type,
        grade: r.grade,
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 120,
      renderCell: params => (
        nearestRef && nearestRef.area ? (
          <Link 
            to={`/areas/${nearestRef.area.id}`} 
            style={{ 
              textDecoration: "none", 
              color: "#1976d2"
            }}
